		...
		if notFound {
			...
			case containerd.Running:
				...
				if err != nil {
					...
					// Start exit monitor.
					c.eventMonitor.startContainerExitMonitor(context.Background(), id, status.Pid, exitCh, c.eventMonitor.exchange)
				}
		...
		if notFound {
			...
			if taskStatus.Status == containerd.Running {
				...
				if err != nil {
					...
					status.Pid = t.Pid()
					c.eventMonitor.startSandboxExitMonitor(context.Background(), meta.ID, status.Pid, exitCh, c.eventMonitor.exchange)
				}
			} else {
